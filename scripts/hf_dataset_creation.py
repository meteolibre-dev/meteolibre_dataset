...
from datasets import (
    ...
    Array3D,
    Array4D,
)  # Choose ArrayXD for flexibility
...
features = Features(
    {
        "radar_back": Array3D(
            shape=(5, array_feature_shape, array_feature_shape),
            dtype=array_feature_dtype,
        ),
        "radar_future": Array3D(
            shape=(4, array_feature_shape, array_feature_shape),
            dtype=array_feature_dtype,
        ),
        "groundstation_future": Array4D(
            shape=(4, array_feature_shape, array_feature_shape, 4),
            dtype=array_feature_dtype,
        ),
        "groundstation_back": Array4D(
            shape=(5, array_feature_shape, array_feature_shape, 4),
            dtype=array_feature_dtype,
        ),
        "ground_height": Array2D(
            shape=(array_feature_shape, array_feature_shape), dtype=array_feature_dtype
        ),  # Assuming height is a 2D array with one column
        ...
        "datetime": Value(
            dtype="string"
        ),  # Assuming this is a string representation of the datetime
...
dataset.push_to_hub(
    "Forbu14/meteolibre_hf_dataset", private=True, token=HfFolder.get_token()
)
